...
dsh-ui-crystal 是**官方 bundle 格式插件**：包内 `dsh.bundle.patch` 声明
...
| 项 | 说明 |
|---|---|
| 插件类型 | DSH **bundle 客户端插件**（`dsh.bundle.patch` + `dsh.client`，浏览器端纯 CSS + 背景切换 UI） |
...
| 背景接口可用 | 浏览器访问 `/ds-crystal/backgrounds` | JSON 文件列表 |
...
| `ui-crystal` 在插件列表里 failed | 旧版本缺 host 入口 | 拉最新代码 `node build.js` 重新部署 |
| 改了 `client.js` 没反应 | HMR 未触发或页面旧 | 等 1 秒或 F5 |
| 改了 bundle patch 没反应 | 启动时才合成 | 重启 `dsh web` |
| 小屏看不到鲸鱼 | 视口 < 720px | 桌面窗口查看 |
| README/文档乱码 | 旧版本编码问题 | 拉取最新代码（已修复为 UTF-8） |
...
- 改样式/背景/按钮：`src/styles.css` 与 `build.js` → `node build.js` → 浏览器自动热更
- 换鲸鱼图：替换 `assets/whale-light.webp` / `assets/whale-dark.webp` → `node build.js`
- 新增白底/带水印背景图：丢进 `assets/backgrounds/` → `node make-transparent.mjs`（自动去白底 + 去右下角水印）
